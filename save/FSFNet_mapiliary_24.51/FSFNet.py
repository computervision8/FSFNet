...
class Encoder(nn.Module):
    ...
    def forward(self, input, predict=False):
        ...
        return output_8, output_16, output_32, inputSize
...
class Decoder(nn.Module):
    ...
    def forward(self, output_8, output_16, output_32, inputSize ):
        height=inputSize[2]
        width =inputSize[3]
        # print(height, width)
        ...
        # 32 downsample rate to 16 downsample rate
        output = F.interpolate(output, size=(int(output_16.size(2)), int(output_16.size(3) )), mode='bilinear', align_corners=True)
        output = torch.cat([output, output_16], 1)
        ...
        # 16 downsample rate to 8 downsample rate
        output = F.interpolate(output, size=(int(output_8.size(2)), int(output_8.size(3) )), mode='bilinear', align_corners=True)
        ...
        # 8 downsample rate to ori downsample rate
        output = F.interpolate(output, size=(int(height), int(width)), mode='bilinear', align_corners=True)
...
class FSFNet(nn.Module):
    ...
    def forward(self, input, only_encode=False):
        if only_encode:  # encoder
            output_8, output_16, output_32, inputSize= self.encoder.forward(input, predict=True)
            return output_32
        else:            # decoder
            output_8, output_16, output_32, inputSize = self.encoder(input)  # predict=False by default
            return self.decoder.forward(output_8, output_16, output_32, inputSize)