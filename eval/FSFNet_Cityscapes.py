...
class Decoder(nn.Module):
    ...
    def forward(self, output_8, output_16, output_32):
        ...
        output = F.interpolate(output, size=(int(output.size(2) * 2), int(output.size(3) * 2)), mode='bilinear', align_corners=True)
        ...
        output = F.interpolate(output, size=(int(output_8.size(2) * 8), int(output_8.size(3) * 8)), mode='bilinear', align_corners=True)
